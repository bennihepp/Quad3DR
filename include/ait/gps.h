...
public:
  ...
  FloatType latitudeRadians() const {
    return ait::degreeToRadians(lat_);
  }

  FloatType longitudeRadians() const {
    return ait::degreeToRadians(long_);
  }
...
public:
  ...
  : a_(a), a_square_(a * a), b_square_(b * b),
    ...
    ref_cos_latitude_(std::cos(ait::degreeToRadians(gps_reference_.latitude()))),
    ref_sin_latitude_(std::sin(ait::degreeToRadians(gps_reference_.latitude()))),
    ref_cos_longitude_(std::cos(ait::degreeToRadians(gps_reference_.longitude()))),
    ref_sin_longitude_(std::sin(ait::degreeToRadians(gps_reference_.longitude()))),
  ...
  GpsCoordinateType convertEcefToGps(const Vector3& ecef) const {
    ...
    const FloatType latitude_degrees = ait::radiansToDegrees(latitude);
    const FloatType longitude_degrees = ait::radiansToDegrees(longitude);
