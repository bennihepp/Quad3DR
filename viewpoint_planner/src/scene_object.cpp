...
SceneObjectAdapted::SceneObjectAdapted() :
    m_zMin(0.0), m_zMax(1.0), m_colorMode(CM_FLAT) {
  }

  void SceneObjectAdapted::heightMapColor(double h, GLfloat* glArrayPos) const {
    if (m_zMin >= m_zMax)
      h = 0.5;
    else{
  ...
  void SceneObjectAdapted::heightMapGray(double h, GLfloat* glArrayPos) const {
    if (m_zMin >= m_zMax)
      h = 0.5;
    else{
