...
{
public:
    PinholeCamera(CameraId id, size_t width, size_t height, const std::vector<double>& params)
...
private:
    void setParameters(const std::vector<double>& params) {
        ...
        intrinsics_(0, 0) = params[0];
        intrinsics_(1, 1) = params[1];
        intrinsics_(0, 2) = params[2];
        intrinsics_(1, 2) = params[3];
    }
...
{
public:
    using CameraMapType = EIGEN_ALIGNED_UNORDERED_MAP(CameraId, PinholeCamera);
...
//        std::cout << "      normal: " << average_normal.transpose() << std::endl;
    ...
    void readCameras(std::istream& in) {
        ...
        while (std::getline(in, line)) {
            ...
            PinholeCamera camera(camera_id, width, height, params);
