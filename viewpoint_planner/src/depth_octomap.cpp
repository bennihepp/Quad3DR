...
void print_query_info(oct::point3d query, oct::OcTreeNode* node) {
  if (node != NULL) {
...
int main(int argc, char** argv)
{
    ...
    oct::OcTree tree (vm["resolution"].as<double>());
...
//  }
    ...
    for (size_t i = 0; i < num_frames_to_extract; ++i) {
        ...
        tree.insertPointCloud(pc, sensor_origin, max_range);
    }
    delete[] depth_data;
    ...
    oct::point3d query (0., 0., 0.);
    oct::OcTreeNode* result = tree.search (query);
    print_query_info(query, result);

    query = oct::point3d(-1.,-1.,-1.);
    ...
    tree.write(vm["map-file"].as<string>() + ".ot");
